...
@Component({
  ...
  template: `
    <nav class="sticky top-0 z-40 w-full bg-white border-b border-gray-100 shadow-sm">
        ...
        <div class="flex justify-between items-center h-20">
          ...
          <div class="flex-shrink-0 flex items-center">
            <a routerLink="/" class="flex items-center gap-2 group">
              <!-- Using a placeholder for the provided golden logo -->
          ...
          <div class="flex items-center space-x-6">
            <a routerLink="/cart" class="flex items-center text-brand-text hover:text-brand-green transition-colors group">
              <span class="text-brand-green font-semibold mr-2">$0.00</span>
              <div class="relative">
            ...
            <!-- Mobile menu button -->
            <button class="md:hidden text-brand-text hover:text-brand-green" (click)="mobileMenuOpen.set(!mobileMenuOpen())">
      ...
      <!-- Mobile Menu -->
      @if (mobileMenuOpen()) {
        <div class="md:hidden bg-white border-t border-gray-100 shadow-inner">
          <div class="px-4 pt-2 pb-4 space-y-1">
            <a routerLink="/" class="block px-3 py-2 text-base font-medium text-brand-text hover:text-brand-green hover:bg-gray-50 rounded-md">Home</a>
            <a routerLink="/products" class="block px-3 py-2 text-base font-medium text-brand-text hover:text-brand-green hover:bg-gray-50 rounded-md">Store</a>
            <a routerLink="/about" class="block px-3 py-2 text-base font-medium text-brand-text hover:text-brand-green hover:bg-gray-50 rounded-md">About Us</a>
            <a routerLink="/contact" class="block px-3 py-2 text-base font-medium text-brand-text hover:text-brand-green hover:bg-gray-50 rounded-md">Contact Us</a>
            @if (auth.isAuthenticated()) {
              <a routerLink="/user/profile" class="block px-3 py-2 text-base font-medium text-brand-text hover:text-brand-green hover:bg-gray-50 rounded-md">My Account</a>
              <button (click)="auth.logout()" class="block w-full text-left px-3 py-2 text-base font-medium text-red-500 hover:bg-red-50 rounded-md">Logout</button>
            } @else {
              <a routerLink="/auth/login" class="block px-3 py-2 text-base font-medium text-brand-green hover:bg-gray-50 rounded-md">Sign In</a>
            }
          </div>
        </div>
      }
